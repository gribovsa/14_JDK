...
public class Main {
    public static final int COUNT_RUNNERS = 3;
    ...
    public static void main(String[] args) {
        ...
        new Thread(new Race(readySignal, startSignal,finishSignal)).start(); //поток трассы

        for (Runner runner : runners) {
            new Thread(runner).start(); //потоки бегунов
        }
    }
